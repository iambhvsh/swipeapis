...
def fetch_yahoo_sync(q: str, region: str, safesearch: str, num_results: int) -> List[Dict[str, Any]]:
    try:
        ...
        results = []
        count = 0
        for r in ddgs.text(query=q, region=region, safesearch=safesearch, backend="yahoo", max_results=num_results):
...
async def fetch_yahoo_results(q: str, region: str = 'us-en', safesearch: str = 'moderate', num_results: int = 10) -> List[Dict[str, Any]]:

...
def fetch_brave_sync(q: str, region: str, safesearch: str, num_results: int) -> List[Dict[str, Any]]:
    try:
        ...
        results = []
        count = 0
        for r in ddgs.text(query=q, region=region, safesearch=safesearch, backend="brave", max_results=num_results):
...
async def fetch_brave_results(q: str, region: str = 'us-en', safesearch: str = 'moderate', num_results: int = 10) -> List[Dict[str, Any]]:
